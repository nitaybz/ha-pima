"""Vendored legacy PIMA Hunter Pro protocol implementation.

This file is taken from https://github.com/deiger/Alarm (pima.py).
Original copyright:
    Copyright (c) 2019 Dror Eiger <droreiger@gmail.com>
    Licensed under GNU GPL v3 or later.

This integration is therefore also GPL-3.0-or-later. Source unchanged except
for this header.

PIMA is a trademark of PIMA Electronic Systems Ltd. The protocol was
reverse-engineered from PIMA's public "General Specification for Home
Automation & Building Management" Ver. 1.15.
"""
...
class Alarm(object):
      ...
      try:
        ...
        for partition, value in enumerate(response[index:index + 16], 1):
          try:
            ...
            # Partition not configured (panel sends 0xFF or similar).
            # Skip so auto-detection can derive the real partition list.
            continue
  ...
  # ─── ha-pima patch ────────────────────────────────────────────────────
  # Frame-aware read. The original library blindly treats the first byte as
  # the length prefix, which fails when we connect mid-frame through a
  # serial-to-Ethernet bridge (Lantronix XPort etc.). We now scan the byte
  # stream for the unique 3-byte signature ``[length, module_id, message]``
  # before committing to a frame parse.
  _VALID_LENGTHS = (0x08, 0x62, 0x7A)
  _VALID_MESSAGES = (0x01, 0x05, 0x0E, 0x0F, 0x19)
  _SYNC_SCAN_BUDGET = 512  # bytes to consume before giving up

  def _read_message(self, max_retries=3, retry_delay=0.5) -> bytes:
    last_error = None
    module_id = ord(self._module_id)
    for attempt in range(max_retries):
      try:
        # Slide a 3-byte window over the stream until it matches a known
        # PIMA frame signature.
        window = bytearray()
        consumed = 0
        synced = False
        while consumed < self._SYNC_SCAN_BUDGET:
          b = self._channel.read(1)
          if not b:
            break
          window.append(b[0])
          consumed += 1
          if len(window) >= 3:
            if (
                window[-3] in self._VALID_LENGTHS
                and window[-2] == module_id
                and window[-1] in self._VALID_MESSAGES
            ):
              synced = True
              break
        if not synced:
          raise Error(
              'Couldn\'t sync to a PIMA frame after {} bytes'.format(consumed)
          )

        length = window[-3]
        head = bytes(window[-3:])  # length + module_id + message (3 bytes)
        # Need (length + 3) total bytes for the whole frame; already have 3.
        remaining = length + 3 - 3
        # Loop because serial-to-Ethernet bridges deliver bytes in many
        # small TCP segments at ~2400 baud — a single socket.recv() will
        # almost always return less than the full frame.
        rest = bytearray()
        while len(rest) < remaining:
          chunk = self._channel.read(remaining - len(rest))
          if not chunk:
        ...
        if len(rest) != remaining:
          if attempt < max_retries - 1:
            ...
            continue
          raise Error(
              'Short frame: got {} of {} expected bytes'.format(len(rest), remaining)
          )
        data = head + bytes(rest)
        ...
        if crc != self._crc(data):
          if attempt < max_retries - 1:
            logging.debug(
                'Sync candidate failed CRC: %s -> 0x%04x (expected 0x%04x). '
                'Retrying sync.', self._make_hex(data), self._crc(data), crc
            )
            time.sleep(retry_delay)
            continue
        ...
        if self._module_id != data[1:2]:
          # Should never happen given how we synced, but keep the check.
          if attempt < max_retries - 1:
            time.sleep(retry_delay)
            continue
